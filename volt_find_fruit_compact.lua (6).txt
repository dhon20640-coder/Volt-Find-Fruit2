...
local function CheckAndHop()
    -- Não faz hop se Factory ou Castle Raid estiver ativo
    if getgenv().GAutoFactory or getgenv().GAutoRaidCastle then return end
...
-- ==================== AUTO FACTORY & CASTLE RAID ====================
local World1,World2,World3=game.PlaceId==2753915549,game.PlaceId==4442272183,game.PlaceId==7449423635
getgenv().GAutoFactory=false
getgenv().GAutoRaidCastle=false
getgenv().GSelectWeapon="Melee"

-- Função para detectar Core
local function GetConnectionEnemies(name)
    local enemies={name}
    for _,model in pairs(ReplicatedStorage:GetChildren())do
        if model:IsA("Model")and model.Name==name and model:FindFirstChild("Humanoid")and model.Humanoid.Health>0 then
            return model
        end
    end
...
-- Auto Equipar Melee
local function weaponSc(style)
    for _,tool in pairs(Player.Backpack:GetChildren())do
        local tooltip=tool:FindFirstChild("ToolTip")
        if tooltip and tooltip.Value==style then
...
local function CheckNearestTeleporter(targetPos)
    ...
    local distToTarget=(targetPos-char.HumanoidRootPart.Position).Magnitude
    
    if minDist<distToTarget and distToTarget>2000 then
        return TableLocations[closestIsland]
    end
    return nil
end

local function RequestEntrance(entrancePos)
    pcall(function()
        local args={"requestEntrance",entrancePos}
...
-- Auto Factory (Core)
local FactoryPos=CFrame.new(387,215,-9468)
task.spawn(function()
    while task.wait(0.3)do
        if getgenv().GAutoFactory and World2 then
            pcall(function()
                ...
                if Core and Core:FindFirstChild("Humanoid")and Core.Humanoid.Health>0 then
                    ...
                    if coreHRP then
                        ...
                        if hrp then
                            -- Bypass se muito longe
                            local dist=(hrp.Position-coreHRP.Position).Magnitude
                            if dist>2000 then
                                local nearestTP=CheckNearestTeleporter(coreHRP.Position)
                                if nearestTP then
                                    RequestEntrance(nearestTP)
                                    task.wait(0.5)
                                end
                            end
                            
                            -- Equipa Melee
                            weaponSc(getgenv().GSelectWeapon)
                            ...
                            -- Tween para Core
                            local tweenDur=math.max((hrp.Position-coreHRP.Position).Magnitude/350,0.05)
                            local tween=TweenService:Create(hrp,TweenInfo.new(tweenDur,Enum.EasingStyle.Linear),{CFrame=coreHRP.CFrame*CFrame.new(0,25,0)})
                            tween:Play()
                    ...
                    -- Core não existe, verifica frutas
                    if not AnyFruitInServer()then
                        -- Sem frutas, continua verificando Core
                    end
                end
            end)
        end
...
-- Auto Castle Raid
local CastleMobs={"Galley Pirate","Galley Captain","Raider Mercenary","Vampire","Zombie","Snow Trooper","Winter Warrior","Lab Subordinate","Horned Warrior"}
local CastlePos=CFrame.new(-5092,315,-3130)

task.spawn(function()
    while task.wait(0.3)do
        if getgenv().GAutoRaidCastle and World3 then
            pcall(function()
                local char=Player.Character
                local hrp=char and char:FindFirstChild("HumanoidRootPart")
                ...
                for _,mob in pairs(Workspace.Enemies:GetChildren())do
                    if mob:FindFirstChild("HumanoidRootPart")and mob:FindFirstChild("Humanoid")and mob.Humanoid.Health>0 then
                        for _,mobName in ipairs(CastleMobs)do
                            if mob.Name==mobName then
                                ...
                                if dist<2000 then
                                    ...
                                    -- Bypass se muito longe do castelo
                                    if(hrp.Position-CastlePos.Position).Magnitude>2000 then
                                        local nearestTP=CheckNearestTeleporter(CastlePos.Position)
                                        if nearestTP then
                                            RequestEntrance(nearestTP)
                                            task.wait(0.5)
                                        end
                                    ...
                                    -- Tween para mob
                                    local tweenDur=math.max((hrp.Position-mobHRP.Position).Magnitude/350,0.05)
                                    local tween=TweenService:Create(hrp,TweenInfo.new(tweenDur,Enum.EasingStyle.Linear),{CFrame=mobHRP.CFrame*CFrame.new(0,25,0)})
                                    tween:Play()
                ...
                -- Se não achou mobs, vai para posição do castelo
                if not foundMob then
                    if(hrp.Position-CastlePos.Position).Magnitude>500 then
                        local tweenDur=math.max((hrp.Position-CastlePos.Position).Magnitude/350,0.05)
                        local tween=TweenService:Create(hrp,TweenInfo.new(tweenDur,Enum.EasingStyle.Linear),{CFrame=CastlePos})
                        tween:Play()
                    end
                end
            end)
